...
return new class extends Migration
{
    ...
    {
        $levelId = DB::table('school_levels')->where('name', 'prebasica')->value('id');
        ...
        DB::table('school_courses')
            ...
            ->each(function ($course): void {
                DB::table('school_courses')->where('id', $course->id)->update([
                    'display_name' => trim($course->grade.' '.$course->section),
                    'updated_at' => now(),
                ]);
            });
    }
    ...
    {
        $level = DB::table('school_levels')->where('name', 'prebasica')->first();
